...
function PropertiesListContent() {
  ...
  const { data, isLoading } = useQuery<{ propertiesData: Property[]; countData: number }>({
  ...
  const filteredProperties = properties.filter((p) => {
    ...
    const typeTerm = selectedType.toLowerCase().trim();
    ...
    const matchesType =
      !typeTerm ||
      p.propertyTitle?.toLowerCase().includes(typeTerm) ||
      p.propertyLocation?.toLowerCase().includes(typeTerm);
  ...
  return (
    <div className="space-y-8 pb-16">
      ...
      <div className="p-4 sm:p-6 rounded-2xl bg-base-200/50 dark:bg-base-200/80 border border-base-content/10 shadow-sm flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4">
        ...
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-11 pr-10 py-2.5 rounded-xl bg-base-100 dark:bg-neutral-800/80 border border-base-content/10 text-sm focus:outline-none focus:ring-2 focus:ring-[#38B6FF]/40 text-base-content"
          />
          {searchTerm && (
            <button
      ...
      {/* Responsive 2-Column Grid (Eliminates oversized stretched cards on desktop) */}
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {[1, 2, 3, 4].map((n) => (
            <div
              key={n}
              className="h-56 rounded-2xl bg-base-200/70 animate-pulse border border-base-content/10 p-5 flex gap-4"
            >
              <div className="w-48 sm:w-56 h-full bg-base-300 rounded-xl flex-shrink-0" />
